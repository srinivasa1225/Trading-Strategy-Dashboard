...
const TradingDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setSignals(signalsResponse.results);
  ...
  <div className="bg-white rounded-lg shadow-sm border">
    ...
    <div className="p-6">
      <div className="space-y-4">
        {(Array.isArray(signals) ? signals : []).map((item, idx) => {
          ...
          return (
            ...
            >
              <div className="flex items-center space-x-4 mb-4 md:mb-0">
                ...
                <div>
                  <div className="font-medium text-lg">{sig.symbol}</div>
                  <div className="text-sm text-gray-600">{sig.time}</div>
                </div>
              </div>
              <div className="flex-1 grid grid-cols-2 gap-4 mb-4 md:mb-0">
                <div>
                  <div className="text-xs text-gray-500">Price</div>
                  <div className="font-medium">${sig.price}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">Volume</div>
                  <div className="font-medium">{analysis.volume}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">EMA50</div>
                  <div className="font-medium">{analysis.ema50}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">EMA200</div>
                  <div className="font-medium">{analysis.ema200}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">MACD</div>
                  <div className="font-medium">{analysis.macd}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">MACD Signal</div>
                  <div className="font-medium">{analysis.macd_signal}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">RSI</div>
                  <div className="font-medium">{analysis.rsi !== null ? analysis.rsi : '-'}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">Volume MA</div>
                  <div className="font-medium">{analysis.volume_ma !== null ? analysis.volume_ma : '-'}</div>
                </div>
              </div>
              <div className="flex flex-col items-end">
                <div className={`inline-block px-3 py-1 rounded-full text-sm font-medium mb-2 ${getSignalColor(sig.signal)}`}>
                  {sig.signal}
                </div>
                <div className="text-sm text-gray-600">Confidence</div>
                <div className="font-bold text-lg">{sig.confidence}%</div>
